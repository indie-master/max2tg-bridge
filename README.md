...
- Опциональный список пользователей, которым разрешены команды и ответы (`TG_ALLOWED_USER_IDS`)
...
5. (Опционально) Telegram user ID пользователей, которым разрешены команды и ответы — для `TG_ALLOWED_USER_IDS` (через запятую). Для одного пользователя также поддерживается старый `TG_ALLOWED_USER_ID`.
...
| `TG_ALLOWED_USER_IDS` | нет | Разрешённые Telegram user ID через запятую, например `123456789,987654321` |
| `TG_ALLOWED_USER_ID` | нет | Совместимость со старой конфигурацией: один разрешённый Telegram user ID |
...
Все команды работают только в супергруппе (`TG_CHAT_ID`). При `TG_ALLOWED_USER_IDS` команды и ответы принимаются только от перечисленных пользователей. Старый `TG_ALLOWED_USER_ID` поддерживается для обратной совместимости.
